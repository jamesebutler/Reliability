// JScript File
        ...
        function ConfirmDelete(actionNumber,rowNum, obj){
            ...
            if (gv !== null){
                ...
                ret = RISharedWS.DeleteAction(actionNumber,OnSucceeded, OnFailed,OnFailed);
...
function displayActionItemWindow(url,title){
    if (url !== null){
		dhtmlmodal.open("actionItem", "iframe", url, title, "width=1000px,height=350px,resize=1,scrolling=1,center=1", "recal");		
...
//GetEmployeeListBySite
    ResourceDDL=obj;
    ret = RISharedWS.GetEmployeeListBySite(siteID,function(arg){EmployeeList = arg;populateResourceList();}, OnFailed,OnFailed);
}